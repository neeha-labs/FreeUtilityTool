...
Base Amount: ₹${gstData.baseAmount.toLocaleString("en-IN")}
Total GST: ₹${gstData.gstAmount.toLocaleString("en-IN")}
CGST: ₹${gstData.cgst.toLocaleString("en-IN")}
SGST/UTGST: ₹${gstData.sgst.toLocaleString("en-IN")}
Total Amount: ₹${gstData.totalAmount.toLocaleString("en-IN")}
    `.trim();
  ...
  const copyResults = () => {
    const content = `Base: ₹${gstData.baseAmount.toLocaleString("en-IN")}, GST: ₹${gstData.gstAmount.toLocaleString("en-IN")}, Total: ₹${gstData.totalAmount.toLocaleString("en-IN")}`;
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-xl border border-slate-800">
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              ...
              <span className="text-xl font-bold">₹{gstData.baseAmount.toLocaleString("en-IN")}</span>
            </div>
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <span className="text-slate-400 text-sm uppercase tracking-wider">Total GST ({rate}%)</span>
              <span className="text-xl font-bold text-indigo-400">₹{gstData.gstAmount.toLocaleString("en-IN")}</span>
            </div>
            <div className="flex justify-between items-center pt-2">
              <span className="text-slate-400 font-bold uppercase tracking-wider">Total Amount</span>
              <span className="text-3xl font-black text-green-400">₹{gstData.totalAmount.toLocaleString("en-IN")}</span>
            </div>
          </div>
        </div>
        ...
        <Table className="border rounded-2xl overflow-hidden shadow-sm">
          ...
          <TableBody>
            <TableRow>
              ...
              <TableCell className="text-right font-medium">₹{gstData.cgst.toLocaleString("en-IN")}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell className="text-slate-600">SGST/UTGST ({rate/2}%)</TableCell>
              <TableCell className="text-right font-medium">₹{gstData.sgst.toLocaleString("en-IN")}</TableCell>
            </TableRow>
            <TableRow className="font-bold bg-slate-50/50">
              <TableCell className="text-indigo-600">Total GST</TableCell>
              <TableCell className="text-right text-indigo-600">₹{gstData.gstAmount.toLocaleString("en-IN")}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
