...
import { Download, Copy, Check, Utensils } from "lucide-react";
...
-----------------------
Bill Amount: ₹${bill.toLocaleString("en-IN")}
Tip Percentage: ${tipPercentage}%
Number of People: ${people}
Total Tip: ₹${results.totalTip.toLocaleString("en-IN")}
Total Bill: ₹${results.totalBill.toLocaleString("en-IN")}
Tip per Person: ₹${results.tipPerPerson.toLocaleString("en-IN")}
Total per Person: ₹${results.totalPerPerson.toLocaleString("en-IN")}
    `.trim();
  ...
  const copyResults = () => {
    const content = `Total Bill: ₹${results.totalBill.toLocaleString("en-IN")}, Total per Person: ₹${results.totalPerPerson.toLocaleString("en-IN")}`;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      <div className="space-y-8">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <Label className="text-slate-700 font-bold">Bill Amount (₹)</Label>
            <span className="font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">₹{bill.toLocaleString("en-IN")}</span>
          </div>
          <Slider value={[bill]} min={0} max={50000} step={100} onValueChange={(v) => setBill(v[0])} />
        ...
        <div className="space-y-4">
          ...
          <Slider value={[tipPercentage]} min={0} max={50} step={1} onValueChange={(v) => setTipPercentage(v[0])} />
        ...
        <div className="space-y-4">
          ...
          <Slider value={[people]} min={1} max={50} step={1} onValueChange={(v) => setPeople(v[0])} />
      ...
      <div className="flex flex-col items-center justify-center space-y-8 bg-slate-50/50 rounded-3xl p-8 border border-slate-100">
        <div className="text-center space-y-4">
          ...
          <h3 className="text-7xl font-black text-indigo-600">₹{results.totalPerPerson.toLocaleString("en-IN")}</h3>
          <div className="inline-flex items-center gap-2 bg-indigo-100 text-indigo-700 px-4 py-2 rounded-2xl font-bold text-lg">
            <Utensils className="w-5 h-5" /> Tip ₹{results.tipPerPerson.toLocaleString("en-IN")} each
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 w-full">
          <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm text-center">
            ...
            <p className="text-lg font-bold text-slate-900">₹{results.totalBill.toLocaleString("en-IN")}</p>
          </div>
          <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm text-center">
            <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">Total Tip</p>
            <p className="text-lg font-bold text-slate-900">₹{results.totalTip.toLocaleString("en-IN")}</p>
          </div>
        </div>
