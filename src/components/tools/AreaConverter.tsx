...
import { Download, Copy, Check, Maximize } from "lucide-react";

type Unit = "Square Meters" | "Square Kilometers" | "Square Feet" | "Acres" | "Hectares";
...
export default function AreaConverter() {
  ...
  const [fromUnit, setFromUnit] = useState<Unit>("Square Meters");
  ...
  const results = useMemo(() => {
    ...
    return (Object.keys(CONVERSION_RATES) as Unit[]).reduce((acc, unit) => {
      acc[unit] = (baseValue * CONVERSION_RATES[unit]).toFixed(6);
      return acc;
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <Select value={fromUnit} onValueChange={(v) => setFromUnit(v as Unit)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="Square Kilometers">Square Kilometers (km²)</SelectItem>
        ...
        <div className="p-6 bg-indigo-50 rounded-3xl border border-indigo-100 space-y-4">
          <h4 className="font-bold text-indigo-900 text-sm uppercase tracking-wider flex items-center gap-2">
            <Maximize className="w-4 h-4" /> Area Info
          </h4>
